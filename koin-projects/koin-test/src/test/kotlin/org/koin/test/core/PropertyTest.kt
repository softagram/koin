...
class PropertyTest : AutoCloseKoinTest() {
    ...
    fun `should load integer value`() {
        startKoin(
            listOf(SimpleModule),
            PropertiesConfiguration(useEnvironmentProperties = true, useKoinPropertiesFile = true)
        )
    ...
    fun `should inject external property`() {
        startKoin(listOf(SimpleModule), PropertiesConfiguration(useKoinPropertiesFile = true))
    ...
    fun `should inject internal property`() {
        startKoin(
            ...
            PropertiesConfiguration(
                extraProperties = mapOf(KEY to VALUE),
    ...
    fun `should inject property - complex module`() {
        startKoin(listOf(ComplexModule), PropertiesConfiguration(useKoinPropertiesFile = true))
        setProperty(KEY, VALUE)
    ...
    fun `should not inject property but get default value as return`() {
        startKoin(listOf(NoPropertyModule), PropertiesConfiguration(useKoinPropertiesFile = true))
    ...
    fun `should overwrite property`() {
        startKoin(listOf(MoreComplexModule), PropertiesConfiguration(useKoinPropertiesFile = true))
        setProperty(KEY, VALUE)
